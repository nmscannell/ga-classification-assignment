...
class DecisionFork:
    ...
    def __init__(self, attr, depth, attr_name=None, default_child=None, branches=None):
    ...
    def max_depth(self):
        return max(subtree.max_depth() for subtree in self.branches.values())

    def get_predictors(self, predictors=[]):
        if predictors.count(self.attr_name) == 0:
            predictors.append(self.attr_name)
        for (val, subtree) in self.branches.items():
            subtree.get_predictors(predictors)
        return predictors
...
def DecisionTreeLearner(dataset):
    ...
    def decision_tree_learning(examples, attrs, depth, parent_examples=()):
        if len(examples) == 0:
            return plurality_value(parent_examples, depth)
        if all_same_class(examples):
            return DecisionLeaf(examples[0][target], depth)
        if len(attrs) == 0:
            return plurality_value(examples)
        A = choose_attribute(attrs, examples)
        tree = DecisionFork(A, dataset.attr_names[A], depth, plurality_value(examples, depth))
        for (v_k, exs) in split_by(A, examples):
            subtree = decision_tree_learning(exs, remove_all(A, attrs), depth+1, examples)
            tree.add(v_k, subtree)
        return tree

    def plurality_value(examples, depth):
        """
        Return the most popular target value for this set of examples.
        (If target is binary, this is the majority; otherwise plurality).
        """
        popular = argmax_random_tie(values[target], key=lambda v: count(target, v, examples))
        return DecisionLeaf(popular, depth)
    ...
    return decision_tree_learning(dataset.examples, dataset.inputs, 0)
...
def compare(algorithms=None, datasets=None, k=10, trials=1):
    ...
    Print results as a table.iris, orings, zoo, restaurant, SyntheticRestaurant(20),
    """
